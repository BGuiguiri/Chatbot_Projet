...
def ask_groq(user_question, conversation_history):
    api_key = st.secrets.get("GROQ_API_KEY", "")
    if not api_key:
        return "Erreur : clé API manquante. Ajoutez GROQ_API_KEY dans les secrets Streamlit."
...
<div class="rh-header">
    ...
    <div class="rh-badge">⚡ Groq · LLaMA 3</div>
</div>
""", unsafe_allow_html=True)
...
if (send or user_input) and user_input.strip():
    ...
    with st.spinner("🤔 L'assistant réfléchit..."):
        answer = ask_groq(question, st.session_state.api_history[:-1])
